...
_unpackinfo(Fontchar *fc, uchar *p, int n)
{
  ...
  for(j=0;  j<=n;  j++){
    ...
    fc->left = p[4];
    fc->width = p[5];
    //    print("[%d] x=%d, top=%d, bottom=%d, left=%d, width=%d\n",
    //	  j, fc->x, fc->top, fc->bottom, fc->left, fc->width);
    fc++;
    p += 6;
  }
}
...
int smaxx; /* subfont max x */
int bmaxx; /* bdf max x */
uchar *sbit;

getxy(uchar *bit, int x, int y, int maxx) {
  int byteperrow = roundup(maxx);
  uchar *p = bit + y*byteperrow + x/8;
  int off = x%8;

  return (*p >> (7-off)) & 1;
}

setxy(uchar *bit, int x, int y, int maxx) {
  int byteperrow = roundup(maxx);
  uchar *p = bit + y*byteperrow + x/8;
  int off = x%8;
...
bdfinit(bdfchar *p, int width, int height) {
  int byteperrow = roundup(width);
  p->width = width;
  p->height = height;
  p->byteperrow = byteperrow;
  p->bit = malloc(byteperrow * p->height);
  memset(p->bit, 0, byteperrow * p->height);
}
...
bdfset(bdfchar *p, uchar *bit, int i) {
  int y, x;

  memset(p->bit, 0, p->byteperrow * p->height);
  for(y=0; y<p->height; y++) {
    for(x=0; x<p->width; x++) {
      uchar c = getxy(bit, fc[i].x+x, y, smaxx);
      if(c)
	setxy(p->bit, x, y, bmaxx);
    }
  }
}
...
bdfgen(bdfchar *p) {
  ...
  for(i=0; i<256; i++) {
    ...
    int y, x;
    for(y=0; y<p->height; y++) {
      for(x=0; x<p->byteperrow; x++)
	print("%02X", p->bit[y*p->byteperrow + x]);
      print("\n");
    }
    print("ENDCHAR\n");
  }
...
main(int argc, char **argv) {
  ...
  maxx = atoi(p+3*12);
  maxy = atoi(p+4*12);
  //  print("ld=%d, minx=%d, miny=%d, maxx=%d, maxy=%d, sizeofdefont=%d\n",
  //	ld, minx, miny, maxx, maxy, sizeofdefont);
  ...
  width = atoi(p+2*12);
  p += 3*12;
  //  print("n=%d, height=%d, width=%d\n", n, height, width);
  fc = malloc((n+1) * sizeof(fc[0]));
  _unpackinfo(fc, p, n);
